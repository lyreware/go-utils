...
func TestCloneByteSlice(t *testing.T) {
	...
	for _, slice := range testSlices {
		...
		if (slice == nil && clone != nil) || (slice != nil && clone == nil) {
			t.Fatalf("CloneByteSlice(%v): got %v", slice, clone)
		}

		if cap(clone) > len(slice) {
			t.Fatalf("CloneByteSlice(%v) expected not greater than %d but got %d", slice, len(slice), cap(clone))
		}

		if len(slice) > 0 && len(clone) > 0 && &slice[0] == &clone[0] {
			t.Fatalf("CloneByteSlice(%v) returned the same slice %v", slice, &clone[0])
		}
	}
}
